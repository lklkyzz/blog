...
class Posts_model extends CI_Model {
    ...
    * 获取文章列表bystatus
    */
    public function getPostsByStatus($page, $size, $status) {
        $this->db->select('p.pid, p.title, p.summary, p.content, c.name as category_name, t.name as tag_name, p.status, p.level, p.comments_count, p.created, p.updated');
    ...
    * 获取文章列表byid
    */
    public function getPostsById($page, $size, $cid) {
        $this->db->select('p.pid, p.title, p.summary, c.name as category_name, t.name as tag_name, , p.status, p.level, p.comments_count, p.created, p.updated');
        $this->db->from('posts p');
        $this->db->join('categories c', 'p.cid=c.cid', 'left');
        $this->db->join('tags t', 'p.tid=t.tid', 'left');
    ...
    * 检查是否重名
    */
    public function checkName($pid, $name) {
        $this->db->select('name');
        $this->db->from('posts');
        $this->db->where('pid !=', $pid);
